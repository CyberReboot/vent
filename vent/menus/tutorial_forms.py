...
class TutorialAddingFilesForm(TutorialForm):
    ...
    def __init__(self, *args, **keywords):
        ...
        provided directory is where Vent will be watching. Just add the desired
        file into the directory and Vent will automatically process those files
        with their respective plugins.

        To change the directory that Vent watches, on the main menu, there's a
        field labeled 'User Data'. Within that directory, there exists a file
        named 'vent.cfg'. Within the file, there is a section titled '[main]'
        with an option 'files'. Just change the value of 'files' to the desired
        directory.


        Example:
        Let's assume that all core tools have been added, built, and have been
        started. Let's also assume there's a plugin that deals with
        *.csv files. If a .csv file is placed into the File Drop directory,
        Vent will recognize the file extension and run the plugin. This also
        works for multiple plugins that deal with the same file type. If there
        were two plugins that dealt with .csv files, Vent would run both on the
        same .csv file.

        In addition, Vent also supports plugin pipelines.
        Let's say there are two plugins: Plugin A processes .csv files and
        Plugin B processes .csv2 files. A .csv file is added to the File Drop
        directory, so Vent will start Plugin A, which will process the file and
...
class TutorialStartingCoresForm(TutorialForm):
    ...
    def __init__(self, *args, **keywords):
        ...
        text = """
        Once a core tool has been added and built, the only thing left is to
        start it, so it can begin doing work.
        ...
        Once the desired tools are selected, press OK and Vent will start up the
        core tools.
        """
