...
class UpdateCoreToolsForm(npyscreen.ActionForm):
    ...
    def create(self):
        ...
        self.add(npyscreen.TitleText, name='Select which tools to update (only core tools are shown):', editable=False)
            ...
            for tool in self.tools_tc[repo]:
                ...
                if self.tools_tc[repo][tool].value:
                    ...
                    if t.startswith('/:'):
                        t = " "+t[1:]
                    t = t.split(":")
                    thr = threading.Thread(target=self.api_action.update, args=(),
                                           kwargs={'name':t[0],
                                                   'branch':t[1],
                                                   'version':t[2]})
