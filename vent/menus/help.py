    ...
    def switch(self, page):
        def popup(page):
            ...
            if page == 'Menu':
                ...
                can install and run. Plugins are developed following a hybrid of
                requirements specified both by Docker and Vent. Vent uses Docker
                to run all plugins so all plugins should be designed to run as a
                ...
                more info on the Core Tools that Vent starts with check out this link:
                ...
                info_str = """
                    ...
                    Built means that each tool has a Docker image successfully built
                    based off the provided specs for that tool/plugin. Enabled/Disabled
                    correspond to user defined settings to enable or disable a tool or
                    set of tools (plugin). Installed means simply that the plugin has
                    been cloned from GitHub and installed to the Vent filesystem.
                    No Docker image has been created yet. Running means that a Docker
                    container has successfully been created from the corresponding
                    Docker image for a specific tool in a Plugin."""
                ...
                info_str = """
                    ...
                    notify you about all tools it has detected. For more information
                    about how Vent detects tools, see our "Building a Plugin" section.
                    You may select or deselect the tools you wish to install as part
                    of your Plugin. When you are done, select OK. If everything
                    works you should get a successful Add. Select OK, to be returned
                    to the main screen!"""
