...
RUN cat $ROOTFS/vent/custom >> $ROOTFS/etc/profile.d/boot2docker.sh

# install dependencies
WORKDIR /tmp
RUN mkdir -p $ROOTFS/usr/local/share/man/man1
RUN tar xf $ROOTFS/vent/dependencies/tinycore-python2/python2.tar
...
RUN rm -rf $ROOTFS/vent/dependencies
RUN rm -rf $ROOTFS/vent/.git
RUN rm -rf $ROOTFS/vent/.gitignore
