...
def test_template_queue():
    ...
    os.environ['HOSTNAME'] = subprocess.check_output(['docker', 'run --name core-template-queue2 -d alpine:latest /bin/sh -c "while true; do echo hello world; sleep 1; done"'])
    file_watch.template_queue("/core.template")
    os.environ['HOSTNAME'] = subprocess.check_output(['docker', 'run --name active-template-queue2 -d alpine:latest /bin/sh -c "while true; do echo hello world; sleep 1; done"'])
    file_watch.template_queue("/collectors.template")
    os.environ['HOSTNAME'] = subprocess.check_output(['docker', 'run --name passive-template-queue2 -d alpine:latest /bin/sh -c "while true; do echo hello world; sleep 1; done"'])
    file_watch.template_queue("/collectors.template")
    os.environ['HOSTNAME'] = subprocess.check_output(['docker', 'run --name visualization-template-queue2 -d alpine:latest /bin/sh -c "while true; do echo hello world; sleep 1; done"'])
    file_watch.template_queue("/visualization.template")
